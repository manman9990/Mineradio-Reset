...
const LOCAL_LIBRARY_FILE = process.env.MINERADIO_LOCAL_LIBRARY_FILE || path.join(UPDATE_WORK_DIR, 'local-library.json');
const LYRIC_CACHE_DIR = process.env.MINERADIO_LYRIC_CACHE_DIR || path.join(UPDATE_WORK_DIR, 'lyrics');
...
const LOCAL_AUDIO_EXTS = new Set(['.mp3', '.flac', '.wav', '.ogg', '.oga', '.m4a', '.aac', '.opus']);
const LOCAL_COVER_EXTS = ['.jpg', '.jpeg', '.png', '.webp'];
const LOCAL_SCAN_MAX_TRACKS = 5000;
...
function normalizeLocalLibrary(raw) {
  const lib = raw && typeof raw === 'object' ? raw : {};
  const tracks = Array.isArray(lib.tracks) ? lib.tracks : [];
  return {
    version: 1,
    root: typeof lib.root === 'string' ? lib.root : '',
    scannedAt: lib.scannedAt || '',
    tracks: tracks.filter(t => t && typeof t.path === 'string').map(t => ({
      id: String(t.id || localTrackId(t.path, t.mtimeMs || 0, t.size || 0)),
      name: String(t.name || ''),
      artist: String(t.artist || '本地音乐'),
      album: String(t.album || ''),
      path: String(t.path || ''),
      fileName: String(t.fileName || path.basename(t.path || '')),
      ext: String(t.ext || path.extname(t.path || '').toLowerCase()),
      size: Number(t.size || 0) || 0,
      mtimeMs: Number(t.mtimeMs || 0) || 0,
      coverPath: t.coverPath ? String(t.coverPath) : '',
      lyricPath: t.lyricPath ? String(t.lyricPath) : '',
...
function readLocalLibrary() {
  try {
    if (!fs.existsSync(LOCAL_LIBRARY_FILE)) return normalizeLocalLibrary(null);
    return normalizeLocalLibrary(JSON.parse(fs.readFileSync(LOCAL_LIBRARY_FILE, 'utf8').replace(/^\uFEFF/, '')));
  } catch (e) {
    return normalizeLocalLibrary(null);
  }
}

function writeLocalLibrary(lib) {
  ensureDirForFile(LOCAL_LIBRARY_FILE);
  const tmp = LOCAL_LIBRARY_FILE + '.tmp';
  fs.writeFileSync(tmp, JSON.stringify(normalizeLocalLibrary(lib), null, 2), 'utf8');
  fs.renameSync(tmp, LOCAL_LIBRARY_FILE);
}

function localTrackId(filePath, mtimeMs, size) {
  return crypto.createHash('sha1').update([path.resolve(filePath || ''), Math.round(Number(mtimeMs) || 0), Number(size) || 0].join('|')).digest('hex').slice(0, 24);
}

function localTitleArtistFromFile(fileName) {
  const base = path.basename(fileName || '', path.extname(fileName || '')).trim();
  return { artist: '本地音乐', name: base || '未命名歌曲' };
}

function findSiblingLocalAsset(dir, stem, exts) {
  for (const ext of exts) {
    const hit = path.join(dir, stem + ext);
    if (fs.existsSync(hit)) return hit;
  }
  return '';
}

function findFolderCover(dir) {
  const names = ['cover', 'folder', 'album'];
  for (const name of names) {
    const hit = findSiblingLocalAsset(dir, name, LOCAL_COVER_EXTS);
    if (hit) return hit;
  }
  return '';
}

function scanLocalMusicDir(root) {
  ...
  while (stack.length && tracks.length < LOCAL_SCAN_MAX_TRACKS) {
    const current = stack.pop();
    let entries = [];
    try { entries = fs.readdirSync(current.dir, { withFileTypes: true }); } catch (e) { continue; }
    const folderCover = findFolderCover(current.dir);
    entries.sort((a, b) => a.name.localeCompare(b.name, 'zh-Hans-CN'));
    for (const entry of entries) {
      const full = path.join(current.dir, entry.name);
      if (entry.isDirectory()) {
        if (current.depth < LOCAL_SCAN_MAX_DEPTH && !/^(node_modules|dist|\.git)$/i.test(entry.name)) {
      ...
      const meta = localTitleArtistFromFile(entry.name);
      tracks.push({
        id: localTrackId(full, st.mtimeMs, st.size),
        name: meta.name,
        artist: meta.artist,
        album: path.basename(current.dir),
        path: full,
        fileName: entry.name,
        ext,
        size: st.size,
        mtimeMs: st.mtimeMs,
        coverPath: findSiblingLocalAsset(current.dir, stem, LOCAL_COVER_EXTS) || folderCover,
        lyricPath: findSiblingLocalAsset(current.dir, stem, ['.lrc', '.txt']),
        duration: 0,
      });
    }
  }
  tracks.sort((a, b) => (a.artist + a.name).localeCompare(b.artist + b.name, 'zh-Hans-CN'));
  return normalizeLocalLibrary({ version: 1, root: resolvedRoot, scannedAt: new Date().toISOString(), tracks });
}

function publicLocalTrack(track) {
  const displayName = path.basename(track.fileName || track.path || track.name || '', path.extname(track.fileName || track.path || '')) || track.name || '未命名歌曲';
  return {
    id: track.id,
    localId: track.id,
    localKey: track.id,
    ...
    ext: track.ext || '',
    cover: track.coverPath ? `/api/local-library/cover?id=${encodeURIComponent(track.id)}` : '',
    hasLyric: !!track.lyricPath,
  };
}

function localLibrarySummary(lib) {
  lib = lib || {};
  ...
  return {
    ...
    source: 'local',
    name: '本地音乐库',
    creator: lib.root ? path.basename(lib.root) : '本机文件夹',
    cover: tracks.find(t => t.coverPath) ? `/api/local-library/cover?id=${encodeURIComponent(tracks.find(t => t.coverPath).id)}` : '',
...
function streamLocalFile(res, req, filePath, contentType) {
  const st = fs.statSync(filePath);
  const range = req.headers.range || '';
  const headers = {
    'Content-Type': contentType || MIME[path.extname(filePath).toLowerCase()] || 'application/octet-stream',
    'Accept-Ranges': 'bytes',
    'Access-Control-Allow-Origin': '*',
  };
  if (range) {
    ...
    const end = match && match[2] ? Math.min(st.size - 1, parseInt(match[2], 10)) : st.size - 1;
...
function sanitizeLyricFilePart(value, fallback) {
  const safe = String(value || '')
    ...
    .replace(/[. ]+$/g, '')
    .slice(0, 96);
  return safe || fallback || 'lyric';
}

function hasTimedLrc(text) {
  return /\[\d{1,2}:\d{1,2}(?:\.\d{1,3})?\]/.test(String(text || ''));
}

function lyricCacheFiles(provider, identity, title, artist) {
  provider = provider === 'qq' ? 'qq' : 'netease';
  identity = String(identity || '').trim();
  const digest = crypto.createHash('sha1').update(provider + '|' + identity).digest('hex').slice(0, 10);
  const label = sanitizeLyricFilePart([artist, title].filter(Boolean).join(' - '), provider + '-' + identity);
  const dir = path.join(LYRIC_CACHE_DIR, provider);
  const suffix = ` [${provider}-${digest}]`;
  let base = `${label}${suffix}`;
  try {
    if (fs.existsSync(dir)) {
      const existing = fs.readdirSync(dir).find(name => name.endsWith(suffix + '.json') || name.endsWith(suffix + '.lrc'));
      if (existing) base = path.basename(existing, path.extname(existing));
    }
  } catch (e) {}
  return {
    lrc: path.join(dir, base + '.lrc'),
    json: path.join(dir, base + '.json'),
  };
}

function readLyricCache(files) {
  try {
    if (fs.existsSync(files.json)) {
      const data = JSON.parse(fs.readFileSync(files.json, 'utf8').replace(/^\uFEFF/, ''));
      const lyricText = fs.existsSync(files.lrc) ? fs.readFileSync(files.lrc, 'utf8').replace(/^\uFEFF/, '') : (data.lyric || '');
      return { ...data, lyric: lyricText, cached: true, cacheFile: files.lrc };
    }
    if (fs.existsSync(files.lrc)) {
      return { lyric: fs.readFileSync(files.lrc, 'utf8').replace(/^\uFEFF/, ''), source: 'lyric-cache', cached: true, cacheFile: files.lrc };
    }
  } catch (e) {
    console.warn('[LyricCache] read failed:', e.message);
  }
  return null;
}

function writeLyricCache(files, data) {
  const lyricText = String((data && data.lyric) || '').replace(/\r\n/g, '\n').trim();
  if (!hasTimedLrc(lyricText)) return false;
  try {
    fs.mkdirSync(path.dirname(files.lrc), { recursive: true });
    fs.writeFileSync(files.lrc, lyricText + '\n', 'utf8');
    fs.writeFileSync(files.json, JSON.stringify({ ...data, lyric: lyricText, savedAt: new Date().toISOString(), cacheFile: files.lrc }, null, 2), 'utf8');
    return true;
  } catch (e) {
    console.warn('[LyricCache] write failed:', e.message);
    return false;
  }
}

async function handleLyricDownload(url) {
  const provider = String(url.searchParams.get('provider') || 'netease').toLowerCase() === 'qq' ? 'qq' : 'netease';
  const title = url.searchParams.get('name') || url.searchParams.get('title') || '';
  ...
  const identity = provider === 'qq' ? (mid || id) : id;
  if (!identity) return { provider, error: provider === 'qq' ? 'Missing QQ song mid or id' : 'Missing song id', lyric: '' };

  const files = lyricCacheFiles(provider, identity, title, artist);
  const cached = readLyricCache(files);
  if (cached && cached.lyric) return { provider, ...cached, saved: false };

  const data = provider === 'qq' ? await handleQQLyric(mid || identity, id) : await handleNeteaseLyric(id);
  const saved = writeLyricCache(files, data);
  return { ...data, cached: false, saved, cacheFile: saved ? files.lrc : '' };
}

function normalizeLocalLyricMatchText(text) {
  return String(text || '')
    .toLowerCase()
    .replace(/[（(【\[].*?[）)】\]]/g, '')
    .replace(/\.(mp3|flac|wav|ogg|oga|m4a|aac|opus)$/i, '')
    .replace(/[\s·・,，。.!！?？'"“”‘’|\-_/]+/g, '');
}

function localTrackSearchMeta(track) {
  const fileStem = path.basename(track.fileName || track.path || track.name || '', path.extname(track.fileName || track.path || ''));
  let title = String(track.name || fileStem || '').trim();
  let artist = String(track.artist || '').trim();
  if (!artist || /本地|local/i.test(artist)) artist = '';
  const parts = fileStem.split(/\s+[-–—_]\s+|[-–—_]/).map(s => s.trim()).filter(Boolean);
  if ((!artist || artist === '本地音乐') && parts.length >= 2) {
    artist = parts[0];
    title = parts.slice(1).join(' - ');
  }
  title = title || fileStem;
  return { title, artist, query: [artist, title].filter(Boolean).join(' ').trim() || fileStem };
}

function localLyricCandidateScore(meta, song) {
  const title = normalizeLocalLyricMatchText(meta && meta.title);
  const artist = normalizeLocalLyricMatchText(meta && meta.artist);
  const name = normalizeLocalLyricMatchText(song && song.name);
  const singer = normalizeLocalLyricMatchText(song && song.artist);
  if (!title || !name) return -100;
  let score = 0;
  if (name === title) score += 90;
  else if (name.includes(title) || title.includes(name)) score += Math.min(name.length, title.length) >= 2 ? 48 : -30;
  else return -80;
  if (artist && singer) {
    if (singer === artist) score += 42;
    else if (singer.includes(artist) || artist.includes(singer)) score += 24;
    else score -= 16;
  }
  if (song && song.provider === 'qq') score += 4;
  return score;
}

async function findOnlineLyricForLocalTrack(track) {
  const meta = localTrackSearchMeta(track);
  const queries = Array.from(new Set([
    meta.query,
    [meta.title, meta.artist].filter(Boolean).join(' ').trim(),
    meta.title,
  ].filter(Boolean)));
  const candidates = [];
  for (const q of queries) {
    try {
      const songs = await handleSearch(q, 8);
      songs.forEach((song, index) => candidates.push({ provider: 'netease', song, score: localLyricCandidateScore(meta, song) - index }));
    } catch (e) {
      console.warn('[LocalLyric] netease search failed:', q, e.message);
    }
    try {
      const songs = await handleQQSearch(q, 8);
      songs.forEach((song, index) => candidates.push({ provider: 'qq', song, score: localLyricCandidateScore(meta, song) - index }));
    } catch (e) {
      console.warn('[LocalLyric] qq search failed:', q, e.message);
    }
  }
  candidates.sort((a, b) => b.score - a.score);
  for (const item of candidates) {
    if (!item || item.score < 42 || !item.song) continue;
    try {
      const data = item.provider === 'qq'
        ? await handleQQLyric(item.song.mid || item.song.songmid || item.song.id, item.song.qqId || item.song.id)
        : await handleNeteaseLyric(item.song.id);
      if (hasTimedLrc(data && data.lyric)) {
        return { ...data, match: { provider: item.provider, id: item.song.id, mid: item.song.mid || '', name: item.song.name || '', artist: item.song.artist || '', score: item.score } };
      }
    } catch (e) {
      console.warn('[LocalLyric] lyric fetch failed:', item.provider, item.song && item.song.id, e.message);
    }
  }
  return { provider: 'local', lyric: '', source: 'local-search-empty', match: null };
}

function localLyricCachePath(track) {
  const id = track && (track.id || track.path) || String(Date.now());
  const digest = crypto.createHash('sha1').update(String(id)).digest('hex').slice(0, 10);
  const label = sanitizeLyricFilePart(path.basename(track.fileName || track.path || track.name || 'local', path.extname(track.fileName || track.path || '')), 'local');
  return path.join(LYRIC_CACHE_DIR, 'local', `${label} [local-${digest}].lrc`);
}

function saveLocalTrackLyric(found, lyricText) {
  const track = found && found.track;
  const lib = found && found.lib;
  const text = String(lyricText || '').replace(/\r\n/g, '\n').trim();
  if (!track || !hasTimedLrc(text)) return { saved: false, lyricPath: '' };
  const audioPath = path.resolve(track.path || '');
  const preferred = path.join(path.dirname(audioPath), path.basename(audioPath, path.extname(audioPath)) + '.lrc');
  const targets = [preferred, localLyricCachePath(track)];
  for (const target of targets) {
    try {
      if (fs.existsSync(target)) {
        track.lyricPath = target;
      } else {
        fs.mkdirSync(path.dirname(target), { recursive: true });
        fs.writeFileSync(target, text + '\n', 'utf8');
        track.lyricPath = target;
      }
      if (lib) writeLocalLibrary(lib);
      return { saved: true, lyricPath: target, source: target === preferred ? 'local-sibling-file' : 'local-cache-file' };
    } catch (e) {
      console.warn('[LocalLyric] save failed:', target, e.message);
    }
  }
  return { saved: false, lyricPath: '' };
}

async function handleLocalLyricDownload(id) {
  const found = findLocalTrack(String(id || ''));
  const track = found.track;
  if (!track || !fs.existsSync(track.path)) return { provider: 'local', error: 'LOCAL_TRACK_NOT_FOUND', lyric: '' };
  if (track.lyricPath && fs.existsSync(track.lyricPath)) {
    return { provider: 'local', lyric: fs.readFileSync(track.lyricPath, 'utf8'), source: 'local-file', cached: true, saved: false, lyricPath: track.lyricPath };
  }
  const data = await findOnlineLyricForLocalTrack(track);
  if (!hasTimedLrc(data && data.lyric)) return { provider: 'local', lyric: '', source: 'local-search-empty', saved: false, match: data.match || null };
  const saved = saveLocalTrackLyric(found, data.lyric);
  return { provider: 'local', ...data, cached: false, saved: saved.saved, lyricPath: saved.lyricPath, source: saved.source || data.source };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/local-library') {
    try {
      const lib = readLocalLibrary();
      sendJSON(res, { ok: true, library: localLibrarySummary(lib), tracks: (lib.tracks || []).map(publicLocalTrack) });
    } catch (err) {
      sendJSON(res, { ok: false, error: err.message, library: localLibrarySummary(null), tracks: [] }, 500);
    }
    return;
  }

  if (pn === '/api/local-library/scan') {
    if (req.method !== 'POST') { sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405); return; }
    try {
      const body = await readRequestBody(req);
      const dir = body.dir || body.path || '';
      if (!dir) { sendJSON(res, { ok: false, error: 'MISSING_LOCAL_LIBRARY_DIR' }, 400); return; }
      const lib = scanLocalMusicDir(dir);
      writeLocalLibrary(lib);
      sendJSON(res, { ok: true, library: localLibrarySummary(lib), tracks: lib.tracks.map(publicLocalTrack) });
    } catch (err) {
      console.error('[LocalLibraryScan]', err);
      sendJSON(res, { ok: false, error: err.message || 'LOCAL_LIBRARY_SCAN_FAILED' }, 500);
    }
    return;
  }

  if (pn === '/api/local-library/tracks') {
    try {
      const lib = readLocalLibrary();
      sendJSON(res, { playlist: localLibrarySummary(lib), tracks: (lib.tracks || []).map(publicLocalTrack) });
    } catch (err) {
  ...
  if (pn === '/api/local-library/url') {
    try {
      const id = url.searchParams.get('id') || '';
      const found = findLocalTrack(id);
      if (!found.track || !fs.existsSync(found.track.path)) { sendJSON(res, { provider: 'local', url: '', error: 'LOCAL_TRACK_NOT_FOUND' }, 404); return; }
      sendJSON(res, {
        provider: 'local',
        source: 'local',
        local: true,
        playable: true,
        url: `/api/local-library/audio?id=${encodeURIComponent(id)}`,
      });
    } catch (err) {
      sendJSON(res, { provider: 'local', url: '', error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/local-library/lyric') {
    try {
      const id = url.searchParams.get('id') || '';
      const found = findLocalTrack(id);
      if (!found.track || !found.track.lyricPath || !fs.existsSync(found.track.lyricPath)) { sendJSON(res, { provider: 'local', lyric: '', source: 'local-empty' }); return; }
      sendJSON(res, { provider: 'local', lyric: fs.readFileSync(found.track.lyricPath, 'utf8'), source: 'local-file' });
    } catch (err) {
      sendJSON(res, { provider: 'local', lyric: '', error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/local-library/lyric/download') {
    try {
      const id = url.searchParams.get('id') || '';
      sendJSON(res, await handleLocalLyricDownload(id));
    } catch (err) {
      console.error('[LocalLyricDownload]', err);
      sendJSON(res, { provider: 'local', lyric: '', error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/local-library/cover') {
    try {
      const id = url.searchParams.get('id') || '';
      const found = findLocalTrack(id);
      if (!found.track || !found.track.coverPath || !fs.existsSync(found.track.coverPath)) { res.writeHead(404); res.end(); return; }
      streamLocalFile(res, req, found.track.coverPath, MIME[path.extname(found.track.coverPath).toLowerCase()] || 'image/jpeg');
    } catch (err) {
      console.error('[LocalLibraryCover]', err);
      res.writeHead(500); res.end();
    }
    return;
  }

  if (pn === '/api/local-library/audio') {
    try {
      const id = url.searchParams.get('id') || '';
      const found = findLocalTrack(id);
      if (!found.track || !fs.existsSync(found.track.path)) { res.writeHead(404); res.end('Local track not found'); return; }
      streamLocalFile(res, req, found.track.path, localAudioContentType(found.track.path));
    } catch (err) {
      console.error('[LocalLibraryAudio]', err);
      res.writeHead(500); res.end();
    }
    return;
  }
  ...
  // ---------- 歌词 ----------
  if (pn === '/api/lyric/download') {
    try {
      const data = await handleLyricDownload(url);
      sendJSON(res, data, data.error ? 400 : 200);
    } catch (err) {
      console.error('[LyricDownload]', err);
  ...
  if (pn === '/api/lyric') {
    try {
      ...
      sendJSON(res, await handleNeteaseLyric(id));
    } catch (err) {
